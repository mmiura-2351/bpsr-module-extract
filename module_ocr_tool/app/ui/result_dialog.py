...
class ResultDialog(tk.Toplevel):
    ...
    def _build(self, candidates: list[ParsedEffectCandidate]) -> None:
        ...
        ttk.Label(frame, text="OCR結果", font=("", 13, "bold")).grid(row=0, column=0, columnspan=4, sticky="w", pady=(0, 10))
        ...
        ttk.Button(frame, text="確定", command=self._confirm).grid(row=button_row, column=2, sticky="e", pady=(12, 0), padx=(0, 4))
        ttk.Button(frame, text="キャンセル", command=self._cancel).grid(row=button_row, column=3, sticky="e", pady=(12, 0))
    ...
    def _create_row(self, frame: ttk.Frame, row: int, index: int, row_data: dict[str, str | list[str]]) -> None:
        ...
        self._rows.append(_RowModel(jp_var=jp_var, effect_var=effect_var, value_var=value_var))
